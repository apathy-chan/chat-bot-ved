...
def get_answer(question, data, current_section=None):
    ...
    return {
        "answer": "Извините, я не знаю ответа на этот вопрос. Попробуйте переформулировать.",
        "section": None
    }
...
def add_request():
    try:
        ...
        if not text or len(text.strip()) < 5:
            return jsonify({'success': False, 'error': 'Текст запроса слишком короткий'})
        new_request = {
        ...
        print(f"New support request added: {new_request['id']}")
        return jsonify({
            'success': True,
            'request': new_request,
        ...
        print(f"Error adding support request: {e}")
...
def notify_user():
    data = request.json
    # Здесь реализуйте отправку уведомления в личный кабинет пользователя
    # Например, через базу данных, очередь, или вызов API
...
def welcome():
    ...
    Ожидает JSON:
    {
        "section_id": "account"   # или другой id раздела, или None/отсутствует, если с главной
    }
    """
    try:
        data = load_data()
        ...
        return jsonify({'message': 'Ошибка при формировании приветственного сообщения.'})
